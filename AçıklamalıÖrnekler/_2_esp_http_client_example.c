...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_HEADER_SENT:
            ...
            break;
        case HTTP_EVENT_ON_HEADER: // HTTP_EVENT_ON_HEADER eventında header_key ve header_value kullanılabilir.
            ESP_LOGD(TAG, "HTTP_EVENT_ON_HEADER, key=%s, value=%s", evt->header_key, evt->header_value);
            break;
        case HTTP_EVENT_ON_DATA: // esp_http_client_on_data_t veri yapısında veriyi alabilir.
...
{
    // Bu fonksiyonda metotlar URL içerisinde yer almaktadır /get /post /put /patch /delete /head vb gibi. 
    ...
    esp_http_client_config_t config = {
        //.url // olursa alttakileri override eder
        .host = CONFIG_EXAMPLE_HTTP_ENDPOINT,  // "httpbin.espressif.cn" string halde URL ismi
        .path = "/get", // Eğer ayarlanmazsa default "/" url + path şeklinde
        .query = "esp", // ? işaretinden sonra yazılacak string (parametreler query string)
    ...
    //*GET (Sunucudan veri alma)
    // TODO: Bunu nasıl parse edip kullanabiliriz? 
    esp_err_t err = esp_http_client_perform(client); // perform mevcut taskı bloklamaktadır. Bunun için wifi'dan ayrı task oluşturulabilir mi ? 
    // Eğer istenirse perform unblocking şekilde gerçekleştirilebiir. 
    // Eğer method ayarlanmadıysa default olarak GET metodu kullanılır. 
    // esp_http_client_method_t
    if (err == ESP_OK) { // Eğer perform başarılı gerçekleştirildiyse (GET request)
    ...
    // user_data local_response_buffer içerisinde yer almaktadır. 

    //*POST (Sunucuya veri gönderirken bu şekilde hazırlamak gerekli!)
    const char *post_data = "{\"field1\":\"value1\"}"; // Json verisini oluştur .
    //#define CONFIG_EXAMPLE_HTTP_ENDPOINT "httpbin.espressif.cn"
    // Bu şekilde yazınca preprocessor stringleri birleştirmektedir. 
    esp_http_client_set_url(client, "http://"CONFIG_EXAMPLE_HTTP_ENDPOINT"/post");
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_header(client, "Content-Type", "application/json");
    esp_http_client_set_post_field(client, post_data, strlen(post_data));
    err = esp_http_client_perform(client); // Post işlemini gerçekleştir.
    if (err == ESP_OK) {
...
{
    // Perform burada kullanılan fonksiyonları kendi içinde çağırmaktadır
...
void app_main(void)
{
    ...
    // Buraya kadar AP bağlanma aşaması yer almakta. 
    }

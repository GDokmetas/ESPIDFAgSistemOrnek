...
#define log_on_err(err, msg) if(err != ESP_OK) ESP_LOGE("WIFI_NETWORK", msg)
...
#define deinit_on_err(err) if(err != ESP_OK) {esp_wifi_deinit(); return err;}
    ...
    #define check_null_ptr(ptr) if(ptr == NULL) {ESP_LOGE("x", "Null Pointer"); return -1;}
#else
...
static wifi_network_disconnect_reason_e wifi_sta_disconnect_reason = WIFI_NETWORK_DISCONNECT_REASON_NOT_DEFINED;
static wifi_network_disconnect_reason_e wifi_ap_disconnect_reason = WIFI_NETWORK_DISCONNECT_REASON_NOT_DEFINED;
static EventGroupHandle_t wifi_network_event_group = NULL;
static wifi_network_sta_static_ip_conf_t static_ip = {0};
static wifi_network_ap_stats_t current_ap_stats = {0};
static wifi_network_sta_stats_t current_sta_stats = {0};
// Private typedefs 
        ...
        switch (event_id)
        {
           ...
           // Bu olay esp_wifi_start() fonksiyonu ESP_OK dördüğünde tetiklenir.
            ...
            case WIFI_EVENT_STA_CONNECTED:
            {
                ...
                strncpy((char*)current_sta_stats.ssid, (char*)sta_connected_event_data->ssid, WIFI_NETWORK_MAX_SSID_LEN);
                memcpy(current_sta_stats.bssid, sta_connected_event_data->bssid, 6);
                current_sta_stats.channel = sta_connected_event_data->channel;
                current_sta_stats.authmode = sta_connected_event_data->authmode;
                current_sta_stats.aid = sta_connected_event_data->aid;
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
            {
                ...
                // ssid, ssid_len, bssid, channel, authmode, aid 
                wifi_sta_disconnect_reason = (wifi_network_disconnect_reason_e)sta_disconnected_event_data->reason;
                #if (WIFI_NETWORK_DEBUG_LOG)
                    char common_msg[COMMON_MSG_SIZE] = "\0";
                    reason_to_string(wifi_sta_disconnect_reason, common_msg);
                    ESP_LOGW("WIFI_NETWORK", "Disconnect Reason: %s", common_msg);
                #endif

                // Bağlantı kopunca mevcut durum bilgilerini sıfırla.
                current_sta_stats.ssid[0] = '\0';
                memset(current_sta_stats.bssid, 0, 6);
                current_sta_stats.channel = 0;
                current_sta_stats.authmode = 0;
                current_sta_stats.aid = 0;
                current_sta_stats.ip = 0;
                current_sta_stats.netmask = 0;
                current_sta_stats.gateway = 0;
            ...
            case WIFI_EVENT_AP_STACONNECTED:
            {
                ...
                current_ap_stats.is_new_sta = true;
                current_ap_stats.last_sta_aid = ap_staconnected_event_data->aid;
                memcpy(current_ap_stats.last_sta_mac, ap_staconnected_event_data->mac, 6);
                current_ap_stats.num_connected_sta++;
            ...
            case WIFI_EVENT_AP_STADISCONNECTED:
            {
                ...
                #endif
                current_ap_stats.num_connected_sta--;
                current_ap_stats.last_sta_aid = 0;
                memset(current_ap_stats.last_sta_mac, 0, 6);
                ...
                // Reason burada switch case yapılacak
                // wifi_err_reason_t tipinde
                wifi_ap_disconnect_reason = (wifi_network_disconnect_reason_e)ap_stadisconnected_event_data->reason;
                #if (WIFI_NETWORK_DEBUG_LOG)
                    char common_msg[COMMON_MSG_SIZE] = "\0";
                    reason_to_string(wifi_ap_disconnect_reason, common_msg);
                    ESP_LOGW("WIFI_NETWORK", "Disconnect Reason: %s", common_msg);
                #endif
    ...
    else if (event_base == IP_EVENT)
    {
        switch (event_id)
        {
            ...
            case IP_EVENT_STA_GOT_IP: // station got IP from connected AP
            {
                ...
                current_sta_stats.ip = got_ip_event_data->ip_info.ip.addr;
                current_sta_stats.netmask = got_ip_event_data->ip_info.netmask.addr;
                current_sta_stats.gateway = got_ip_event_data->ip_info.gw.addr;
                #if (WIFI_NETWORK_DEBUG_LOG)
            ...
            case IP_EVENT_AP_STAIPASSIGNED: // soft-AP assign an IP to a connected station
            {
                ...
                current_ap_stats.last_sta_ip = ap_staipassigned_event_data->ip.addr;
                //? burada ip atanan sta'nın mac adresi ne olacak? Nasıl değerlendirilebilir?
                #if (WIFI_NETWORK_DEBUG_LOG)
...
esp_err_t wifi_network_config_static_ip(wifi_network_sta_static_ip_conf_t *static_ip_conf)
{
    ...
    check_null_ptr(static_ip_conf);
    static_ip.enabled = static_ip_conf->enabled;
    if(static_ip_conf->ip == 0 || static_ip_conf->netmask == 0 || static_ip_conf->gateway == 0)
    {
        ESP_LOGE("WIFI_NETWORK", "Invalid IP configuration");
        static_ip.enabled = false;
        return ESP_ERR_INVALID_ARG;
    }

    static_ip.ip = static_ip_conf->ip;
    static_ip.netmask = static_ip_conf->netmask;
    static_ip.gateway = static_ip_conf->gateway;
    static_ip.dns1 = static_ip_conf->dns1;
    static_ip.dns2 = static_ip_conf->dns2;
    return err;
}
esp_err_t wifi_network_init(EventGroupHandle_t event_group)
{
    ...
    if (static_ip.enabled) // ! Statik IP Modu (test edilecek)
    {
        #if (WIFI_NETWORK_DEBUG_LOG)
        ESP_LOGI("WIFI_NETWORK", "Static IP enabled");
        #endif
        err = esp_netif_dhcpc_stop(wifi_netif);
        wifi_error_check(err, "func: wifi_network_init() -> esp_netif_dhcpc_stop()");
        return_on_err(err);

        esp_netif_ip_info_t ip_info = {
            .ip.addr = static_ip.ip,
            .netmask.addr = static_ip.netmask,
            .gw.addr = static_ip.gateway
        };
        esp_netif_set_ip_info(wifi_netif, &ip_info);

        ip_addr_t dns_ip;
        dns_ip.type = IPADDR_TYPE_V4;
        dns_ip.u_addr.ip4.addr = static_ip.dns1;
        dns_setserver(0, &dns_ip);
        dns_ip.u_addr.ip4.addr = static_ip.dns2;
...
esp_err_t wifi_network_softap_mode(void)
{
    ...
    uint8_t ap_ssid_len = strnlen((char*)ap_config.ap.ssid, WIFI_NETWORK_MAX_SSID_LEN);
    if (ap_ssid_len == 0)
    {
            ...
            return ESP_ERR_INVALID_ARG;
        #else
            //? Bu kısımda auth mode open de olabilir.
            strncpy((char*)ap_config.ap.ssid, WIFI_NETWORK_DEFAULT_AP_SSID, WIFI_NETWORK_MAX_SSID_LEN);
        #endif
    }

    ap_config.ap.ssid_len = ap_ssid_len; // SSID len doğru hesaplandı!
...
esp_err_t wifi_network_scan_mode(void)
{
    ...
    wifi_scan_config_t scan_config = {
        ...
        .scan_time = {
            .active = {
                .min = WIFI_NETWORK_DEFAULT_MIN_SCAN_TIME,
                .max = WIFI_NETWORK_DEFAULT_MAX_SCAN_TIME
            },
        }
    };
...
{
    nw_err_t err;
    check_null_ptr(sta_ssid);
    if(ssid_len > WIFI_NETWORK_MAX_SSID_LEN)
    {
        ESP_LOGE("WIFI_NETWORK", "SSID length is too long");
        return WIFI_NETWORK_ERROR;
    }
    if (ssid_len < 2)
    {
        ESP_LOGE("WIFI_NETWORK", "SSID length is too short");
        return WIFI_NETWORK_ERROR;
    }
...
{
    nw_err_t err;
    check_null_ptr(sta_pass);
    if(pass_len > WIFI_NETWORK_MAX_PASS_LEN)
    {
        ESP_LOGE("WIFI_NETWORK", "Password length is too long");
        return WIFI_NETWORK_ERROR;
    }
...
{
    esp_err_t err = ESP_OK;
    check_null_ptr(ap_stats);
    memcpy(ap_stats, &current_ap_stats, sizeof(wifi_network_ap_stats_t));
    return err;
}

//*---------------------------------UTILITY--------------------------------------------//

static nw_err_t secure_string_copy(char *dest, char *src, size_t dest_size)
{
...
{
    // Burada kullanıcının girdiği string verisine dikkat!
    nw_err_t err = WIFI_NETWORK_OK;
    check_null_ptr(ipaddr);
    check_null_ptr(addr);

    int i; 
    char ipaddr_tmp[16]; 
    strncpy(ipaddr_tmp, ipaddr, 16); 
    char *token = strtok(ipaddr_tmp, "."); 
    for (i = 0; i < 4; i++) 
    { 
        *addr |= (uint32_t)atoi(token) << (i * 8); 
        token = strtok(NULL, "."); 
    }

#if (WIFI_NETWORK_DEBUG_LOG)
    ESP_LOGI("WIFI_NETWORK", "Converted IP: %s -> %lu", ipaddr, *addr);
#endif
...
{
    nw_err_t err = WIFI_NETWORK_OK;
    check_null_ptr(ip_config);
    check_null_ptr(ip);
    check_null_ptr(netmask);
    check_null_ptr(gateway);
    check_null_ptr(dns1);
    check_null_ptr(dns2);

    err = wifi_network_convert_str_ip_u32(ip, &ip_config->ip);
    #if (WIFI_NETWORK_DEBUG_LOG)
    log_on_err(err, "func: wifi_network_generate_static_ip_conf() -> Wrong IP format");
    #endif
    return_on_err(err);
    err = wifi_network_convert_str_ip_u32(netmask, &ip_config->netmask);
    #if(WIFI_NETWORK_DEBUG_LOG)
    log_on_err(err, "func: wifi_network_generate_static_ip_conf() -> Wrong Netmask format");
    #endif
    return_on_err(err);
    err = wifi_network_convert_str_ip_u32(gateway, &ip_config->gateway);
    #if(WIFI_NETWORK_DEBUG_LOG)
    log_on_err(err, "func: wifi_network_generate_static_ip_conf() -> Wrong Gateway format");
    #endif
    return_on_err(err);
    err = wifi_network_convert_str_ip_u32(dns1, &ip_config->dns1);
    #if(WIFI_NETWORK_DEBUG_LOG)
    log_on_err(err, "func: wifi_network_generate_static_ip_conf() -> Wrong DNS1 format");
    #endif
    return_on_err(err);
    err = wifi_network_convert_str_ip_u32(dns2, &ip_config->dns2);
    #if(WIFI_NETWORK_DEBUG_LOG)
    log_on_err(err, "func: wifi_network_generate_static_ip_conf() -> Wrong DNS2 format");
    #endif
    return_on_err(err);
...
static void reason_to_string(wifi_network_disconnect_reason_e reason, char* reason_string)
{
    ...
    default: snprintf(reason_string, COMMON_MSG_SIZE, "DISCONNECT_REASON_NOT_DEFINED (%d)", reason); break;
    }
}